...
def loss(p,index_2_use):
    ...
    if model_type == 'ANN':
        if regularization == 'L1':
            training_loss=np.mean(errors**2.0)+GAMMA_L1*sum([abs(i) for i in p]) 
        elif regularization == 'L2':
            training_loss=np.mean(errors**2.0)+GAMMA_L2*sum([i**2 for i in p])
    else:
